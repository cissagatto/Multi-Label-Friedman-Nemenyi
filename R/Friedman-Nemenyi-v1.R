...
#
##############################################################################
diretorios_1 <- function(name_dir){
  ...
  FolderFN = paste(pasta, "/", name_dir, sep="")
...
##############################################################################
#
# RANK =  caminho absoluto do arquivo
# paste(FolderRanks, "/", files[i], sep="")
# NAMES = apenas o nome do arquivo
#
##############################################################################
Friedman_Nemenyi_v1 <- function(path, name_dir){
  
  retorno = list()

  pastas = diretorios_1(name_dir)
  ...
  while(i<=length(names)){
    ...
    nome_1 = paste(pastas$FolderTXTs,
  ...
  Friedman = fpValues[-1,]
  Hyphotesis = Hypho[-1,]
  Critical_Distances = CD[-1,]
  Treatments = treatments[-1,]
  Observations = Observations[-1,]
  Confidence_Levels = CL[-1,]
  Degrees = Dg[-1,]
  ...
  values = cbind(ChiSquare,
    ...
    Critical_Distances = Critical_Distances$CriticalDistance,
    Treatments = Treatments$Treat,
    Observations = Observations$Obs,
    Confidence_Levels = Confidence_Levels$ConfidenceLevel,
    Degrees = Degrees$Degrees)
  
  setwd(pastas$FolderFN)
